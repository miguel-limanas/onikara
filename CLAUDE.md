...
Act and tell for reversible documentation, text cleanup, routing, structure refinement, and small internal naming improvements.

Propose and wait before structural decisions: changing the main visual direction, changing the central game concept, renaming important concepts, removing large systems, changing phase priorities, changing MVP scope, or making decisions that could erase the dark fantasy/Oni identity.

### Maintenance cadence
<!-- From Batch E Q2. Options: weekly /review | on-demand only | both. -->
Both: weekly `/review` plus on-demand maintenance when new decisions, ideas, artifacts, or direction changes appear.

Weekly review should inspect progress, blockers, pending decisions, next steps, and consistency between narrative, system, and interface.
...
- Do not erase the dark fantasy/Oni identity of Onikara.
- Do not turn Onikara into a generic SaaS dashboard.
- Do not exaggerate anime styling so much that the strategic/tactical tone disappears.
- Do not add systems that are too complex before the MVP proves the core experience.
- Do not change central decisions without recording the reason.
- Do not lose the separation between player, master/admin, and system-operating layers.
