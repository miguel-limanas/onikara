...
Operator-facing verbs. Each command is a thin spec: input, files to load, files to update, what to surface. The agent reads the spec, executes against the brain, and reports back per `pm-brain/CLAUDE.md § Operating loop`.
...
| [`/strategy-check`](./strategy-check.md) | Drift check between recent decisions / hypotheses / ingestion and `pm-brain/knowledge/strategy.md` |
...
- Every command ends by surfacing 2-4 bullets per `pm-brain/CLAUDE.md § Operating loop § 7`.
...
- Commands respect `pm-brain/CLAUDE.md § Operating preferences § Autonomy mode`. Under `propose and wait`, drafts are presented for approval before saving.
- File paths in these specs are written from the repository root and point into `pm-brain/`.
